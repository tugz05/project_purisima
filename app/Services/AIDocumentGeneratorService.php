...
class AIDocumentGeneratorService
{
    ...
    private function buildPrompt(
        ...
        $hasOtherRequestData = false;
        foreach ($requestData as $key => $value) {
            if (isset($definedKeys[$key])) {
                continue;
            }
            if ($value !== null && $value !== '' && (! is_string($value) || trim($value) !== '')) {
                $hasOtherRequestData = true;
                break;
            }
        }

        if ($hasOtherRequestData) {
            $prompt .= "\nADDITIONAL INFORMATION PROVIDED BY RESIDENT (ELABORATE ON THESE WHERE RELEVANT):\n";
            foreach ($requestData as $key => $value) {
                if (isset($definedKeys[$key])) {
                    continue;
                }
    ...
    public function generateCertificateContent(\App\Models\Transaction $transaction): string
    {
        ...
        $inputFieldDefinitions = \App\Models\DocumentType::normalizeDynamicInputFields($transaction->documentType->required_fields);

        return $this->generateDocumentContent($documentType, $residentData, $requestData, $requiredDocuments, $inputFieldDefinitions);
    }
