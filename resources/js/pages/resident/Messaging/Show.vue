...
// Computed properties
const currentUser = computed(() => props.conversation.resident);
...
const sendMessage = async () => {
    ...
    try {
        await router.post(`/resident/messaging/conversations/${props.conversation.id}/messages`, {
...
const handleTyping = () => {
    if (!isTyping.value) {
        isTyping.value = true;
        router.post(`/resident/messaging/conversations/${props.conversation.id}/typing/start`, {}, {
            preserveState: true,
            preserveScroll: true,
        });
    }
    ...
    typingTimeout.value = setTimeout(() => {
        if (isTyping.value) {
            ...
            router.post(`/resident/messaging/conversations/${props.conversation.id}/typing/stop`, {}, {
                preserveState: true,
                preserveScroll: true,
            });
        }
                ...
                <div class="border-t border-gray-200 p-4">
                    <div class="flex items-end gap-3">
                        <div class="flex-1">
                            <div class="relative">
                                <Textarea
                                    ...
                                    rows="1"
                                    class="resize-none rounded-full border-gray-300 focus:border-green-500 focus:ring-green-500 pr-24"
                                    @input="handleTyping"
                        ...
                        <Button
                            ...
                            :class="messageContent.trim() ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-300'"
                        >
                            <Send class="h-4 w-4" />
                        </Button>
                    </div>
